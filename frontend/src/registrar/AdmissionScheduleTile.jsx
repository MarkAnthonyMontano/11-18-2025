...
const ScheduleHoverTile = () => {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const yearsRes = await axios.get("http://localhost:5000/get_school_year/");
                setSchoolYears(yearsRes.data);

                const semRes = await axios.get("http://localhost:5000/get_school_semester/");
                setSchoolSemester(semRes.data);

                const activeRes = await axios.get("http://localhost:5000/active_school_year");
    ...
    useEffect(() => {
        ...
        const filtered = schedules.filter((s) => {
            const matchesSearch =
                s.proctor.toLowerCase().includes(lowerQuery) ||
                s.building_description.toLowerCase().includes(lowerQuery) ||
                s.room_description.toLowerCase().includes(lowerQuery);

            const matchesBuilding =
                selectedBuilding === "" || s.building_description === selectedBuilding;

            return matchesSearch && matchesBuilding;
        });

        setFilteredSchedules(filtered);
    }, [searchQuery, selectedBuilding, schedules]);
    ...
    return (
        ...
        >
            ...
            {/* School Year, Semester & Building in 1 row */}
            <TableContainer
                component={Paper}
                sx={{
                    maxWidth: "100%",
                        ...
                        <FormControl size="small" sx={{ minWidth: 200 }}>
                            ...
                            >
                                {schoolYears.length > 0 ? (
                                    ...
                                    <MenuItem disabled>School Year not found</MenuItem>
                                )}
                            </Select>
                        </FormControl>
                    </Box>
                        ...
                        <FormControl size="small" sx={{ minWidth: 200 }}>
                            ...
                            >
                                {schoolSemester.length > 0 ? (
                                    ...
                                    <MenuItem disabled>School Semester not found</MenuItem>
                                )}
                            </Select>
                        </FormControl>
                    </Box>
